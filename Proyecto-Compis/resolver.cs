...
namespace Proyecto_Compis
{
    public class Resolver
    {
        public  List<string> CambiarPostfijo(List<PropiedadesDePalabras> TokensOperando)
        {
            ...
            {
                if (TokensOperando[i].Nombre == "NUMERO" )
                {
        ...
        public  int evaluarResultado(List<string> posfija)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < posfija.Count; i++)
                {
                    ...
                    {
                        pilaResultado.Push((int)(Convert.ToInt32(posfija[i]) - 0));
                    }
                }
            ...
            {
                return Convert.ToInt32(pilaResultado.Pop());
            }
            return Convert.ToInt32(pilaResultado.Pop());
        }
        public  int operador(int p, int p_2, string p_3)
        {
            switch (p_3)
            {
                case "+":
